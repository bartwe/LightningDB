...
public sealed class LightningEnvironment : IDisposable {
    ...
    ///     Any attempt to set a size smaller than the space already consumed by the environment will be silently changed to
    ...
    public Stats EnvironmentStats {
        get {
            mdb_env_stat(Handle(), out var nativeStat).ThrowOnError();
            return new() {
    ...
    public EnvironmentInfo Info {
        get {
            mdb_env_info(Handle(), out var nativeInfo).ThrowOnError();
            return new() { MapSize = nativeInfo.me_mapsize.ToInt64(), LastPageNumber = nativeInfo.me_last_pgno.ToInt64(), LastTransactionId = nativeInfo.me_last_txnid.ToInt64() };
        }
    }
    ...
    ///     Note:
    ///     Transactions may not span threads; a transaction must only be used by a single thread. Also, a thread may only have
    ///     a single transaction.
    ///     Cursors may not span transactions; each cursor must be opened and closed within a single transaction.
    /// </summary>
    /// <param name="parent">
    ///     If this parameter is non-NULL, the new transaction will be a nested transaction, with the transaction indicated by
    ///     parent as its parent.
    ///     Transactions may be nested to any level.
    ///     A parent transaction may not issue any other operations besides BeginTransaction, Abort, or Commit while it has
    ///     active child transactions.
    /// </param>
    ...
    /// </summary>
    /// <param name="path">
    ///     The directory in which the copy will reside. This directory must already exist and be writable but
    ...
    ///     buffered.
    ///     MDB always flushes the OS buffers upon commit as well, unless the environment was opened with
    ///     EnvironmentOpenFlags.NoSync or in part EnvironmentOpenFlags.NoMetaSync.
    /// </summary>
    /// <param name="force">
    ///     If true, force a synchronous flush. Otherwise if the environment has the
    ///     EnvironmentOpenFlags.NoSync flag set the flushes will be omitted, and with MDB_MAPASYNC they will be asynchronous.
    /// </param>
